...
save "${user_dir}/temp_data_reg3.dta", replace
duplicates drop Trddt_date, force
gen abst = abs(t)
egen mint = min(abst) if abst != 0
gen abslogmint = abs(log10(mint))
gen t_root = sign(t)*(abs(t))^(1/abslogmint) if t != 0
replace t_root = 0 if t == 0
graph twoway (line t_root Trddt_date, msize(0.1) ///
              xtitle("Trading date", size(medium)) ///
              ytitle("High Polynomial function of time", size(medium))), ///
       xlabel(, labsize(tiny) angle(vertical) grid) ///
       ylabel(, labsize(small) grid) ///
       name(graph4_1, replace)
graph export "${user_dir}/graph4_1.png", replace
use "${user_dir}/temp_data_reg3.dta", replace
quietly {
rename y y_stock
...
graph twoway (scatter y_mean_daily_interval Trddt_date, msize(0.1) ///
	   ...
	   xlabel(, labsize(tiny) angle(vertical) grid) ///
       ylabel(, labsize(small) grid) ///
       name(graph3_1, replace)
graph export "${user_dir}/graph3_1.png", replace

esttab reg1 using "${user_dir}/table_results_2007-2010_high_polynomial.tex", ///
...
graph twoway (line t_root Trddt_date, msize(0.1) ///
       ...
       xlabel(, labsize(tiny) angle(vertical) grid) ///
       ylabel(, labsize(small) grid) ///
       name(graph4_2, replace)
graph export "${user_dir}/graph4_2.png", replace
use "${user_dir}/temp_data_reg3.dta", replace
quietly {
rename y y_stock
egen mean_y_stock = mean(y_stock), by(Stkcd)
...
graph twoway (scatter y_mean_daily_interval Trddt_date, msize(0.1) ///
	   ...
	   xlabel(, labsize(tiny) angle(vertical) grid) ///
       ylabel(, labsize(small) grid) ///
       name(graph3_2, replace)
graph export "${user_dir}/graph3_2.png", replace

esttab reg1 using "${user_dir}/table_results_2013-2016_high_polynomial.tex", ///
        replace se star(* 0.10 ** 0.05 *** 0.01) r2 nogaps noconstant compress ///
...
graph twoway (line t_root Trddt_date, msize(0.1) ///
       ...
       xlabel(, labsize(tiny) angle(vertical) grid) ///
       ylabel(, labsize(small) grid) ///
       name(graph4_3, replace)
graph export "${user_dir}/graph4_3.png", replace
use "${user_dir}/temp_data_reg3.dta", replace
quietly {
rename y y_stock
egen mean_y_stock = mean(y_stock), by(Stkcd)
...
graph twoway (scatter y_mean_daily_interval Trddt_date, msize(0.1) ///
	   ...
	   xlabel(, labsize(tiny) angle(vertical) grid) ///
       ylabel(, labsize(small) grid) ///
       name(graph3_3, replace)
graph export "${user_dir}/graph3_3.png", replace

esttab reg1 using "${user_dir}/table_results_2019-2023_high_polynomial.tex", ///
        replace se star(* 0.10 ** 0.05 *** 0.01) r2 nogaps noconstant compress ///
